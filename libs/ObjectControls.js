...
THREE.ObjectControls = function(camera, domElement, objectToMove) {
  ...
  //amsci: this is not a proper zoom
    // function zoomIn() {
  ...
  //amsci: new zoom functions
    var zoomVector = new THREE.Vector3();

    function zoomIn() {
      camera.getWorldDirection(zoomVector);
      camera.position.add(zoomVector.normalize().multiplyScalar(zoomSpeed));
    }
  
    function zoomOut() {
      camera.getWorldDirection(zoomVector);
      camera.position.sub(zoomVector.normalize().multiplyScalar(zoomSpeed));
    }
    ...
    //amsci: modified wheel function
    function wheel(e) {
      var delta = e.wheelDelta ? e.wheelDelta : e.deltaY * -1;
      if (delta > 0 && camera.position.distanceTo(mesh.position) > minDistance) {
        zoomIn();
      } else if (delta < 0 && camera.position.distanceTo(mesh.position) < maxDistance) {
        zoomOut();
      }
    }
